export default async function handler(req, res) {
  ...
  try {
    ...
    const fullText = `Устройтесь поудобнее и закройте глаза. Сделайте три глубоких вдоха, с каждым выдохом отпуская прошлое.

Представьте перед собой дорогу, которая ведет к ${userText}. Солнце мягко освещает ваш путь, наполняя вас теплом и энергией. Вы чувствуете под ногами ${userText}.

Идя по этой дороге, вы встречаете человека, который многое для вас значит - это ${userText}. Он смотрит на вас с гордостью и говорит: "Я вижу, как ты изменилась. Теперь ты ${userText}.

За поворотом дороги открывается прекрасный вид. Вы видите ${userText}. Это место символизирует вашу новую жизнь, где вы будете заниматься ${userText}.

Вы чувствуете, как внутри растет ощущение ${userText}. Это чувство наполняет каждую клеточку вашего тела.

Сделайте глубокий вдох этой новой энергии. На выдохе медленно откройте глаза, унося с собой ясность своего пути. ${userText}`;
